...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        embeddings_var = tf.get_variable('embeddings', initializer=self.pretrained_embeddings)
    ...
    def add_prediction_op(self):
        ...
        Wh = tf.get_variable('Wh', shape=(self.config.hidden_size, self.config.n_classes), initializer=xavier_weight_init())
        b = tf.get_variable('b', shape=(self.config.hidden_size))
        bh = tf.get_variable('bh', shape=(self.config.n_classes))

        h = tf.nn.relu(tf.matmul(x, W) + b)
        h = tf.nn.dropout(h, self.dropout_placeholder)
        pred = tf.matmul(h, Wh) + bh
