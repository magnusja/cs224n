...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        to_pad = max_length - len(sentence)
        mask = [True] * len(sentence) + [False] * to_pad
        labels = labels + [zero_label] * to_pad
        sentence = sentence + [zero_vector] * to_pad
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        embed_var = tf.Variable(self.pretrained_embeddings)
        embeddings = tf.nn.embedding_lookup(embed_var, self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o = tf.nn.dropout(o, self.dropout_placeholder)
                preds.append(tf.matmul(o, U) + b2)
